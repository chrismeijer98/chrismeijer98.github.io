// ============================================================
// Small DOM + UX helpers used across pages
// ============================================================
(function () {
  const h = (tag, attrs = {}, children = []) => {
    ...
    const kids = [].concat(children).filter((c) => c !== null && c !== undefined && c !== false);
  ...
  const initials = (name) =>
    String(name || '')
      .trim().split(/\s+/).map((p) => p[0] || '').join('').toUpperCase().substring(0, 2) || 'HP';

  const genSessionCode = (name) => {
    const rand = Math.random().toString(36).substring(2, 6).toUpperCase();
    return `${initials(name)}-${rand}`;
  };
  ...
  // Centralized icons (Lucide SVGs inlined)
  const ICONS = {
    ...
    send:         '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m22 2-7 20-4-9-9-4 20-7z"/></svg>',
    ...
    coach:        '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 11a5 5 0 1 0-10 0v6h10z"/><path d="M12 2v3"/><path d="m4.9 4.9 2.1 2.1"/><path d="m19.1 4.9-2.1 2.1"/><path d="M3 21h18"/></svg>',
    ...
    pen:          '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4z"/></svg>',
    link:         '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07L11.7 5.17"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07L12.3 18.83"/></svg>',
    sparkles:     '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m12 3-1.9 5.8L4.3 10.7l5.8 1.9L12 18.5l1.9-5.9 5.8-1.9-5.8-1.9z"/></svg>',
  ...
  window.qsa   = qsa;
  window.HopUtil = {
    h, qs, qsa, escapeHtml, initials, genSessionCode, formatDate, formatDateLong,
    toast, copyText, getHashParams, getQueryParams, ICONS,
  };
})();
